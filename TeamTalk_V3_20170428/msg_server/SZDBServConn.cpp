...
void SZDBServConn::HandlePdu(SZPduBase* pPdu)
{
...
void SZDBServConn::_HandleLoginResponse(SZPduBase* pPdu)
{
	IM::Login::IMLoginRes rsp;
	rsp.ParseFromArray(pPdu->GetBodyData(),pPdu->GetBodyLength());
	IM::BaseDefine::UserInfo user_info = rsp.user_info();
	string login_name = user_info.user_nick_name();
	log("login_name: %s",login_name.c_str());
	SZImUserManager* manager = SZImUserManager::Instance();
	SZImUser* pImUser = manager->GetImUserByLoginName(login_name);
	if(!pImUser){
		return;
	}
